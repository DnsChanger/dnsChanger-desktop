...
function LoadingCom() {
    const notes: string[] = [
        " برنامه نیازی به باز گذاشتن نیست! فقط سرور را فعال کنید.",
        "این برنامه به  صورت متن باز  می باشد.",
        "برای استفاده بهینه از این برنامه، آخرین ورژن مرورگر خود را داشته باشید."
    ];
    const note = notes[Math.floor(Math.random() * notes.length)];
    return (
        <div dir='rtl' className='flex flex-col overflow  gap-5 justify-center items-center h-screen py-5' >
            <div className='loader'></div>
            <div className='text-center'>
                <h1 className="text-1xl font-bold text-gray-200 w-60">{note}</h1>
            </div>
            <div>
                <p className="text-sm  mt-2 animate-pulse" style={{ "color": " #fff3cd" }}>در حال بارگذاری ...</p>
            </div>
