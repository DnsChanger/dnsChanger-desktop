...
export function ServerInfoCardComponent(prop: Prop) {
  ...
  function getPing() {
    setPing(0)
    window.ipc.ping(serversStateContext.selected).then(res => res.success && setPing(res.data.time))
  }
      ...
      <div className={'dark:bg-[#262626] bg-base-200 h-[189px] w-[362px] mt-5 rounded-[23px]'}>
        <div className={'absolute left-[90px] top-[110px] flex flex-col gap-2'}>
          <div className={'flex flex-rows gap-2'}>
            ...
            <h1 className={'text-2xl mt-1 font-[balooTamma] text-[#7487FF]'}>{LL.pages.home.homeTitle()}</h1>
          </div>
          ...
          <div className={'flex flex-rows gap-2 justify-center'}>
            {prop.loadingCurrentActive ? (
              <div className={'flex flex-rows gap-2'}>
                ...
                <span className={'text-[#7B7B7B]'}>Fetching current active...</span>
              </div>
            ) : (
              <span></span>
            )}
  ...
  const isConnect = serversStateContext.currentActive?.key == serversStateContext.selected.key
  const name =
  ...
  return (
    <div className="dark:bg-[#262626] bg-base-200 h-[189px] w-[362px] mt-5 rounded-[23px]">
      <div className={'grid grid-cols-2 gap-4 mt-5 text-[#434343] dark:text-[#A6A6A6] '}>
        <div className={'flex flex-col gap-2'}>
          <h3 className={'font-semibold text-gray-500'}>Name</h3>

          <div className={'w-100 text-center flex flex-row gap-2 justify-center'}>
            <div className="flex items-center gap-3 p-2 rounded-[10px] dark:bg-gray-900 bg-gray-300">
              ...
              <span className="ml-1 inline-flex items-baseline text-sm">
                <span className="font-medium text-slate-900 dark:text-slate-200 ">{name || 'Unknown'}</span>
              </span>
            </div>
          </div>
        </div>
        <div className={'flex flex-col gap-2 text-center  justify-center'}>
          ...
          <div className={'w-100 flex flex-row gap-1   justify-center text-center'}>
            <Button
              color="ghost"
              size="sm"
              ...
              <span className="ml-1 inline-flex items-baseline text-sm">
                <span className="font-medium text-slate-900 dark:text-slate-200">{ping}</span>
              </span>
              <TfiReload />
            </Button>
          </div>
        ...
        <div className={'flex flex-col gap-2 text-center  justify-center'}>
          ...
          <div className={'w-100 flex flex-row gap-2   justify-center text-center'}>
            {isCopyAdds ? (
              <Button color="success" size="sm" className="flex items-center gap-3">
                <span className="ml-1 inline-flex items-baseline text-sm">
                  <span className="font-medium text-slate-900 dark:text-slate-200 normal-case">Copied!</span>
                </span>
              </Button>
            ) : (
              <Button
                ...
                className="flex items-center gap-3  border-1 border-gray-300 dark:border-gray-900 bg-gray-300 dark:bg-gray-900 dark:hover:bg-gray-800 dark:hover:border-none"
                onClick={() => {
                  navigator.clipboard.writeText(serversStateContext.selected.servers.join(','))
                  setIsCopyAdds(true)
                }}>
                <span className="ml-1 inline-flex items-baseline text-sm">
