...
function DetailsModal(prop: Props): JSX.Element {
  ...
  function onError(_event: Electron.IpcRendererEvent, arg1: { message: string }) {
    setOpenDialog(false)
  }
  ...
  function updateProgressInfo(transferred, total) {
    ...
    setProgressLabel(`Progress: ${formattedTransferred} / ${formattedTotal} (${progress}%)`)
  }
    ...
    <Dialog open={openDialog} size={'xl'} handler={() => {}} className="bg-[#282828] rounded-2xl">
      ...
      <DialogFooter className={'justify-center '}>
        <Button
          ...
          <CgSoftwareDownload size={20} className={'mr-1'} /> close and downloading in background
        </Button>
      </DialogFooter>
    </Dialog>
  )
